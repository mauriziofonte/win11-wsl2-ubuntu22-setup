...
This project is intended to guide the developer through the installation, configuration and _workflow optimization_ of a _LAMP Stack_ that is targeted on Windows 11 (and also on Windows 10, see below), with _WSL2_ and **Ubuntu 24.04**, with a _LAMP Stack_ implemented via _native systemd services_ instead of containers/virtualization.
...
Docker Desktop for Windows can be installed at any time. If you enable WSL2 _after_ having Docker Desktop installed, you can still use Docker Desktop with WSL2, as it will automatically detect the WSL2 VM and use it as the backend (not personally tested, maybe it's only a matter of enabling the _Use WSL 2 based engine_ option in the _General_ settings of Docker Desktop).
...
1. Install the **LAMP Stack** along with **Docker** (author's choice)
